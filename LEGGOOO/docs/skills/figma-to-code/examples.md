...
get_metadata(123:456) → "Button/Primary/Large"
get_design_context(123:456) → colors/iris/950, padding 16px 24px
Search: Glob("components/ui/button.tsx") → Found
Read: Has variant="primary" and size="lg"
Output:
...
get_metadata(789:012) → "ProductCard" with Image, Title, Description, Price, Button
get_design_context(789:012) → Card styling, color tokens
Check: components/ui/card.tsx exists → Use composition
Output:
...
get_metadata(100:200) → Page with Stats Grid showing revenue, users
Recognize: Needs backend API for dynamic data
Ask user about backend implementation approach
Ask user:
...
get_metadata(456:789) → "StatusBadge" with variants: success, warning, error
get_design_context(456:789) → Different colors per variant
Check: No exact match in components/ui/badge.tsx
Ask user if should extend Badge or create new component
User approves creating new component
Output:
...
get_metadata(999:111) → Settings page with tabs: Profile, Notifications, Security
get_design_context for each section
Check existing: PageLayout, Tabs, Form components available
Generate using composition
Output:
